...
ZoteroSort = {
	...
	buildSystemPrompt() {
		return [
			"You file an academic paper into the best folder of a Zotero library. The user message gives the existing folders as hierarchical paths, the folder the paper is currently filed in (if any), and the paper's metadata and an excerpt.",
			"",
			"Rules:",
			"- Strongly prefer an existing folder, but only when the paper is clearly on that folder's topic, not merely adjacent. When several existing folders fit, choose the most specific (deepest) one.",
			"- Propose a NEW folder only when no existing folder fits. Never create a folder whose scope substantially overlaps an existing one — file into that folder instead. A new name is short (1-3 words), matches the existing naming style (capitalization, singular/plural), names a recurring theme (not this one paper), does not duplicate an existing folder, and does not nest deeper than the library already does.",
			'- "path", "parent", and every "alternatives" entry must be copied character-for-character from the existing-folders list; never paraphrase, re-case, or invent a path. A folder not in the list does not exist.',
			"- Never recommend, and never list as an alternative, the folder the paper is already filed in.",
			"- If no folders exist yet, set existing=false, choose a short general name, and leave parent and alternatives empty.",
			"",
			"Return ONLY a JSON object inside a ```json code block with these keys:",
			"{",
			'  "existing": true | false,',
			'  "path": "existing folder path, copied exactly (when existing=true; else empty)",',
			'  "name": "new folder name (when existing=false; else empty)",',
			'  "parent": "existing folder path to nest under, or empty for top-level (when existing=false)",',
			'  "reason": "one short sentence naming the deciding topic; do not restate the title",',
			'  "alternatives": ["up to 3 other existing folder paths, best first; [] if none"]',
			"}",

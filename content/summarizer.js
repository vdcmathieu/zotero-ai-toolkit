...
/**
 * AI Paper Summarizer — core module.
 *
 * Loaded into the bootstrap scope by bootstrap.js and exposed as
 * Zotero.AISummarizer so the preferences pane can reach it.
 *
 * Security notes:
...
#aaaaaa | Other | Future research, open questions, context that fits nowhere else`,
	...
	getPref(key) {
		try {
			return Zotero.Prefs.get("aisummarizer." + key);
		}
		catch (e) {
			return undefined;
		}
	...
	_requireApiKey() {
		...
		if (!apiKey) {
			this._notify("AI Summarizer", "No API key configured for " + this._providerLabel(provider)
				+ ". Opening settings…");
	...
	async summarizeSelected(window) {
		if (this._running) {
			this._notify("AI Summarizer", "A summarization is already running — please wait for it to finish.");
			return;
		}

		let targets = this._getSelectedRegularItems(window);
		if (!targets.length) {
			this._notify("AI Summarizer", "Select one or more articles (or their attachments) first.");
			return;
		}
		...
		pw.changeHeadline("AI Summarizer — " + this._providerLabel(provider));
		pw.show();
		...
		try {
			for (let item of targets) {
				...
				try {
					...
					Zotero.debug("AI Summarizer error for item " + item.id + ": " + e);
					line.setError();
	...
	async categorizeSelected(window) {
		if (this._running) {
			this._notify("AI Summarizer", "Another AI task is already running — please wait for it to finish.");
			return;
		}
		...
		if (categories.length < 2) {
			this._notify("AI Summarizer", "The category list in the settings is invalid — reset it to the built-in taxonomy.");
			Zotero.Utilities.Internal.openPreferences(this.paneID);
			return;
		}
		...
		this._running = true;
		let pw = new Zotero.ProgressWindow({ closeOnClick: false });
		pw.changeHeadline("AI Highlight Categorizer — " + this._providerLabel(provider));
		pw.show();
		...
		try {
			for (let item of targets) {
				...
				try {
					...
					Zotero.debug("AI Summarizer categorize error for item " + item.id + ": " + e);
					line.setError();
					line.setText("Failed: " + shortTitle + " — " + this._shortError(e));
				}
			}
	...
	async _getItemText(item) {
		...
		try {
			...
			Zotero.debug("AI Summarizer: full-text extraction failed: " + e);
		}
